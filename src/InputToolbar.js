...
export default class InputToolbar extends React.Component {
    ...
    componentDidUpdate(prevProps) {
        if (this.props.renderResponder1 !== prevProps.renderResponder1) {
            if (prevProps.renderResponder1 == null) {
                const toolbarHeight = this.props.renderResponder1
                    ? this.state.height + (Platform.OS == "ios" ? 24 : 26)
                    : this.state.height - (Platform.OS == "ios" ? 24 : 26);
                this.setState({
    ...
    handleComposerSizeChange(size) {
        // console.log("😀", { size });
        this.setState({
            height: size.height + (this.props.renderResponder1 ? 45 : 20)
        });
    }
    ...
    renderComposer() {
        ...
        return (
            <Composer
                {...this.props}
                handleComposerSizeChange={size =>
                    this.handleComposerSizeChange(size)
                }
            />
        );
    }
